...
class TestBSpline1D:
    """Tests for the 1-D degree-1 (hat) B-spline."""

    def test_peak_at_zero(self) -> None:
        """B(0) should be 1."""
        assert bspline1d_evaluate(0.0) == 1.0

    def test_zero_at_boundary(self) -> None:
        """B(±1) should be 0."""
        assert bspline1d_evaluate(1.0) == 0.0
        assert bspline1d_evaluate(-1.0) == 0.0
...
class TestBSpline1DDerivative:
    ...
    def test_positive_side(self) -> None:
        ...
        assert bspline1d_derivative(0.1) == -1.0
    ...
    def test_zero_outside_support(self) -> None:
        """B'(t) = 0 for |t| >= 1."""
        assert bspline1d_derivative(1.0) == 0.0
        assert bspline1d_derivative(-1.0) == 0.0
...
class TestBSpline3D:
    """Tests for the 3-D trilinear B-spline."""

    def test_peak_at_center(self) -> None:
        """B_3d(center) = 1."""
        c = (1.0, 2.0, 3.0)
        assert bspline3d_evaluate(c, c, 1.0) == 1.0

    def test_zero_outside_support(self) -> None:
        """B_3d should be 0 when any axis is outside support."""
        c = (0.0, 0.0, 0.0)
        # One cell width away in x
        assert bspline3d_evaluate((1.0, 0.0, 0.0), c, 1.0) == 0.0
        # Far away
    ...
    def test_partition_of_unity(self) -> None:
        ...
        On a uniform grid with cell width w, the point (x, y, z)
        overlaps with at most 2^3 = 8 cell centres (the corners
        of the grid cell containing the point). The sum of their
        B-spline values should be exactly 1.
        """
        w = 0.5
...
class TestBSpline3DGradient:
    ...
    def test_gradient_zero_at_center(self) -> None:
        ...
        The hat function has a cusp at t=0 and we define
        B'(0) = 0, so all three components vanish.
        """
        c = (0.0, 0.0, 0.0)
...
class TestStencilEnumeration:
    ...
    def test_3x3x3_corner_stencil(self) -> None:
        """A corner cell in a 3x3x3 grid should have 8 neighbors."""
        dmin = (0.0, 0.0, 0.0)
        dmax = (3.0, 3.0, 3.0)
        cells = _make_uniform_grid_cells(3, dmin, dmax)
        offsets, neighbors = enumerate_stencils(cells, dmin, dmax, 3, 0)
        ...
        # Corner cell touches 2x2x2 = 8 cells (including self)
        assert len(stencil_0) == 8
